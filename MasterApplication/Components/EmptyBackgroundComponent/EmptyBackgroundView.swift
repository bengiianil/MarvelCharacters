...
class EmptyBackgroundView: GenericBaseView<EmptyBackgroundViewData> {
    ...
    private func getLabelPackComponentData() -> LabelPackComponentData {
        return LabelPackComponentData()
            .setSubtitleLabelDistributionData(by: LabelPackDistributionData().setContentMode(by: .center).setTextAlignment(by: .center).setNumberOfLines(by: 2).setLineBreakMode(by: .byTruncatingTail).setFont(by: MainFont.regular(16).value))
            .setTitleLabelDistributionData(by: LabelPackDistributionData().setContentMode(by: .center).setTextAlignment(by: .center).setNumberOfLines(by: 1).setLineBreakMode(by: .byTruncatingTail).setFont(by: MainFont.bold(24).value))
            .setSpacing(by: 10)
